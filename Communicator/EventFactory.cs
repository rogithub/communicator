...
namespace Communicator
{
    ...
    internal class EventFactory : IEventFactory
    {
        ...
        {
            return Connection.InvokeAsync<Guid>(EventNames.SendXml, eventName, SerializationXml.Serialize(meta), data);
        }

        public Task<Guid> Binary(string eventName, byte[] data, MetaData meta) 
        {            
            return Connection.InvokeAsync<Guid>(EventNames.SendBinary, eventName, SerializationXml.Serialize(meta), data);
        }
        
        public Task<Guid> String(string eventName, string data, MetaData meta) 
        {            
            return Connection.InvokeAsync<Guid>(EventNames.SendString, eventName, SerializationXml.Serialize(meta), data);
        }

        public Task<Guid> StringTo(string eventName, string connectionId, string data, MetaData meta) 
        {            
            return Connection.InvokeAsync<Guid>(EventNames.SendStringTo, connectionId, eventName, SerializationXml.Serialize(meta), data);
        }
    }
}
